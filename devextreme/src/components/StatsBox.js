  ...
  render() {
    ...
    this.element.innerHTML = `
      <div><strong>FPS</strong>: <br/>
        current: ${data.fpsCurrent} <br/>
        average: ${data.fpsAverage} <br/>
        min: ${data.fpsMin} <br/>
      </div>
      <div><strong>Memory</strong>: ${data.memoryUsed} Mb</div>
      <div><strong>Time to render</strong>: ${Math.round(data.time)} ms</div>
      ${data.label ? `<div><strong>Status</strong>: ${data.label}</div>` : ""}
    `;
  }
}
